...
/**
 * สไลเดอร์ช่วงราคาแบบ 2 หัว — ลากต่อเนื่องลื่น ไม่หลุด
 * วิธี: ตอน pointerdown ผูก pointermove/up ที่ `window` ครั้งเดียว (handler ตัวเดิมตลอดการลาก)
 * แล้วถอดตอน pointerup — re-render ระหว่างลากจึงถอด listener ไม่ได้ (เคยพังเพราะ useEffect/useCallback deps)
...
export default function PriceRange({
  ...
  // ค่าล่าสุด — อ่านในตัวจับ event ที่ผูกครั้งเดียวต่อการลาก
  const latest = useRef({ lo, hi, onChange, min, max, step });
  latest.current = { lo, hi, onChange, min, max, step };
  // ตัวถอด listener ที่ใช้งานอยู่ (เผื่อ unmount กลางการลาก)
  ...
  // ถอด listener หากคอมโพเนนต์ถูก unmount ระหว่างลาก
  useEffect(() => () => cleanup.current?.(), []);

  // แตะบนราง = เลื่อนหัวที่ใกล้กว่า
  function onTrackDown(e: React.PointerEvent<HTMLDivElement>) {
    if ((e.target as HTMLElement).dataset.thumb) return; // จับหัวอยู่
  ...
  const thumbCls = 'absolute top-1/2 h-6 w-6 -translate-x-1/2 -translate-y-1/2 cursor-grab touch-none rounded-full border-2 border-gold bg-white shadow-[0_1px_4px_rgba(0,0,0,0.25)] outline-none transition-transform focus-visible:ring-2 focus-visible:ring-gold/40 active:cursor-grabbing active:scale-110';
  ...
  return (
    <div ref={trackRef} className="relative h-6 cursor-pointer touch-none select-none" onPointerDown={onTrackDown}>
      ...
      />
      {/* หัวต่ำสุด — JSX inline (DOM เสถียร) */}
      <div
        data-thumb="lo" role="slider" tabIndex={0}
        aria-valuemin={min} aria-valuemax={max} aria-valuenow={lo} aria-label="ราคาต่ำสุด"
      ...
      <div
        ...
        aria-valuemin={min} aria-valuemax={max} aria-valuenow={hi} aria-label="ราคาสูงสุด"
