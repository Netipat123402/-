...
- ✅ **DONE `1707f8b`** — listing sort control (native select pill, เจ้าของเคาะ A): เรียง ใหม่ล่าสุด/ราคาต่ำ→สูง/สูง→ต่ำ/ยอดนิยม บนแถวผลลัพธ์ · backend รองรับอยู่แล้ว (public.dto @IsIn) ขาดแค่ UI · verify จริง end-to-end (เรียงถูก+คง filter+มือถือพอดี). Icon เพิ่ม `sort` · dict TH/EN 5 คีย์
