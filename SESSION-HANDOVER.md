...
- **📊 สถานะ: 🎉 STAGE1 จบ 100%** — ✅ P0–P10 · ✅ **P11 QA** (build ผ่าน 10 routes · tsc 0 · QA sweep **27/27 combos สะอาด** = 9 หน้า×3 จอ ไม่มี overflow/รูปแตก/error โค้ด · เหลือแค่ Google Fonts fetch benign) → **พร้อม STAGE2 (สลับ Notify/ไทย · P12–P13)**
- ⚠️ **บทเรียน (สำคัญ):** อย่าสั่ง `next build` ในโฟลเดอร์ apps/web-v2 ขณะ dev server (:3002) รัน → build ทับ `.next` ทำ dev เสิร์ฟ 404/500 · กู้: ลบ `.next` + start dev ใหม่ให้ regen (ครั้งนี้กู้แล้ว :3002 ปกติ) · ถ้าจะ build ใช้โฟลเดอร์แยก/หยุด dev ก่อน
- **🧩 reusable components (ครบ):** AboutBenefits · FreshSection · FreshListings · Testimonials · StatsBand(variant) · FaqAccordion · FaqSection · ArticleCard · ScrollReveal · icons — 10 หน้าประกอบจาก component reuse สูง
- **🗺 หน้าครบแล้ว (10 route):** `/`(Home 9sec) · `/property`(list) · `/property/[slug]`(detail) · `/agent` · `/about` · `/contact` · `/add-property` · `/blog`(list) · `/blog/[slug]`(detail) · Nav+Footer ทุกหน้า
- **🧩 reusable components:** `AboutBenefits` · `FreshSection`(viewAll) · `FreshListings`(client) · `Testimonials` · `StatsBand`(variant) · `FaqAccordion`(client) · `FaqSection` · `icons.tsx` — reuse Home/Agents/About/Contact · ⏳ ค้าง P4: Video section, more-in-location กรองย่าน
- **commit web-v2:** `0147282`(P0) · `05cb4ab`(P1 Nav) · `6b5622b`(P1 Footer) · `00b825f`(§1 Hero) · `a5f2ed5`(§2 Featured) · `6f261b5`(§3 About) · `4348a6b`(§4 Cities) · `7eb5da8`(§5 Steps) · `2c25caf`(§6 Fresh+filter) · `08d4dc5`(§7 Testimonials marquee) · `9ef4953`(§8 Stats) · **`f2a33a2`(§9 Insights — ปิด P2)** · ⚠️ **หน้า Home = pixel-clone ครบแล้ว** (Nav/Footer/Home 9 sec) · property list ยัง v0 (ทำ P3)
- **🧩 components web-v2:** `Nav` `Footer` · `FeaturedCard`(มินิมอล Home bento) · `PropertyCard`(rich canonical · Fresh+/property) · `FreshListings`(client · filter tabs) · `TCard`/`Benefit` inline · marquee CSS ใน globals.css
...
- **👉 งานต่อไปทันที:** เข้า **P2 Home** — ไล่ทีละ section แบบ pixel-match (วัด→เสนอ→เคาะ→สร้าง→เทียบ 3 จอ): hero → Featured 6 → About collage → Cities 6 → Steps → Fresh+filter → Testimonials 6 → Stats band → Insights → (CTA band ทำใน Footer แล้ว)
