...
- **🟢 track C (กำลังทำ · ⚠️ owner เปลี่ยนแนว → i18n 2 ภาษา): แปล web-admin เป็น bilingual (อังกฤษหลัก + ไทยรอง สลับได้)**
  ...
  - 🔴 **KNOWN ISSUES เหลือ (ต้องแก้ก่อนถือว่าเสร็จ):**
    1. ~~master-data ไทยเสมอ~~ ✅ **เสร็จ (properties)** — เหลือ LEAD_SOURCE + property-requests type = ทำตอน migrate entity นั้น (consumer อยู่หน้าที่ยังไม่ migrate)
    2. **component ยังไม่แปล** (= C-shared 2/2 ถัดไป) — `DocumentSection` (บัตรประชาชน/แนบเอกสาร/ยังไม่มีเอกสาร), `ActivityTimeline`/ประวัติ (เปลี่ยนสถานะ.../แก้ไขทรัพย์...), GlobalSearch, NotificationBell, QuickAddProperty → **โผล่ในทุกหน้า detail** = ภาษาปนทุกที่
  ...
  - **📋 แผนที่เหลือ (เสนอ · ให้ owner เคาะก่อนเริ่ม):**
    ...
    - **C-shared 2/2 (ถัดไป)** · แปล component ร่วม: DocumentSection(บัตรประชาชน/แนบเอกสาร/ยังไม่มีเอกสาร) · ActivityTimeline/ประวัติ(เปลี่ยนสถานะ.../แก้ไขทรัพย์... — ต้องแตะ backend summary หรือ i18n token ฝั่ง FE เหมือน thaiifyActivity) · GlobalSearch(placeholder "ค้นหา / คำสั่ง…" โผล่ทุกหน้า) · NotificationBell · QuickAddProperty(⚠️ dead code ไม่ถูก import — เช็คก่อนว่าจะลบหรือแปล) — จบครั้งเดียวครอบทุก detail
