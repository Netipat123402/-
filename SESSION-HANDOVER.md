...
- **🟢 track C (กำลังทำ · ⚠️ owner เปลี่ยนแนว → i18n 2 ภาษา): แปล web-admin เป็น bilingual (อังกฤษหลัก + ไทยรอง สลับได้)**
  ...
  - **📋 แผนที่เหลือ (เสนอ · ให้ owner เคาะก่อนเริ่ม):**
    ...
    - **C-entities** (ทีละ entity ตาม CYCLE): ~~owners~~ ✅ (`a883f28`) → ~~leads/ผู้สนใจ~~ ✅ (`41ff852`) → ~~appointments/นัดหมาย~~ ✅ (`5645b31` · helper relUntil i18n + time keys · fix bug double-แล้ว) → **customers (ถัดไป)** → contracts → property-requests · แต่ละ entity = list+detail+form · **ต่อ entity: ติเก่า→รูปเทียบ 3 จอ→เคาะ→ทำ→verify authed 3 จอ→audit ภาษาปน→commit** · ⚠️ property-requests → เก็บ PROPERTY_TYPE display ที่ค้าง
