...
## 4) 🎯 งานถัดไป (owner สั่งไว้ · ยังไม่ทำ · ทำตาม CYCLE §2)
### ⭐ A) แยกหมวดข้อมูลใน DETAIL ที่ยังติดกัน (§10 category split — ต่อจากฟอร์ม)
ไล่สแกน **detail ทุกหน้า** หาชุดข้อความคนละความหมายที่ติดกันแต่ควรแยก (เช่น **นัด: วันที่+สถานที่ ติดกัน** · หน้าอื่นอีกเยอะ) → แยกเป็น InfoGroup/บรรทัด · **เสนอ before/after + รูป 3 จอ + เหตุผลตามกฎ ก่อนแก้** · เสร็จแล้วบันทึกกฎ
### ⭐ B) per-device data subset (§11 — ต่อยอด · ยังไม่ทำใน detail)
แต่ละ device/orientation โชว์ **หัวข้อตามความสำคัญของหน้านั้น ไม่ต้องครบทุกหัวข้อ** (สุดท้ายจิ้มเข้า detail ดูลึกอยู่ดี) → เสนอว่า มือถือ/iPad ตั้ง/iPad นอน/คอม ควรโชว์หัวข้อไหน + **ถามก่อน + เหตุผล + รูป 3 จอ** · เป้า = ค้นหา/สแกนง่ายสุดต่อ device
...
---
**เริ่ม session ใหม่:** อ่านไฟล์นี้ + `DESIGN-SYSTEM.md` → ทำงานต่อ §4 (A แยกหมวด detail / B per-device subset) หรือรับงานใหม่จากเจ้าของ · **ยึด CYCLE §2 เป๊ะ: ทีละหน้า · ติเก่า+เสนอใหม่+รูป 3 จอ · รอเคาะ · เคาะค่อยทำ · verify authed · commit · ต่อหน้า**
