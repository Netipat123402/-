...
🔵 **เหลือ:** C-backend 2/2 = **notification title/body** (ต้อง DB migration) — **C-system ครบ 5/5 + C-backend 1/2 (activity summary) เสร็จแล้ว**
...
- **🟢 track C (กำลังทำ · ⚠️ owner เปลี่ยนแนว → i18n 2 ภาษา): แปล web-admin เป็น bilingual (อังกฤษหลัก + ไทยรอง สลับได้)**
  ...
  - **📋 แผนที่เหลือ (เสนอ · ให้ owner เคาะก่อนเริ่ม):**
    ...
    - **C-backend (ลึก · ปิดท้าย) — owner เคาะ "แยก 2 เฟส" (activity ก่อน · notification ทีหลัง แยก DB risk):**
      ...
      - 🔵 **C-b2 · notification title/body (ถัดไป · ต้อง owner เคาะก่อนเริ่ม — มี DB migration):** `Notification` **ไม่มี metadata column** → ต้องเพิ่ม `title_key`/`body_key`/`params` (manual SQL ADD COLUMN + `migrate resolve --applied` · **ห้าม `migrate dev`** — schema-drift landmine) · 21 notify call site (NotifyInput +titleKey/bodyKey/params) · FE `NotificationBell` อ่าน key → t() · fallback stored title/body (row เก่า) · notify pattern มี `thaiDateTime`/ชื่อ/code/status params (ดู [`notification.service.ts`](apps/api/src/modules/notification/notification.service.ts) NotifyInput)
...
- 🔑 **push commit ค้าง (~161 local · ต้อง token · +7 commit จาก session นี้: 5 C-system + handover + C-b1 activity)** · 🖼 `apps/web-public/public/hero.jpg` (ถ้ายัง)
